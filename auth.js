...
export default async function handler(req, res) {
  ...
  if (action === 'debug') {
    ...
    let dbMsg = '';
    try {
      const r = await fetch(`${SUPABASE_URL}/rest/v1/user_profiles?select=id&limit=1`, { headers: H });
      dbOk = r.ok;
      dbMsg = r.status + '';
    ...
    return res.status(200).json({
      hasUrl: !!SUPABASE_URL,
      hasKey: !!SUPABASE_KEY,
      urlOk: SUPABASE_URL?.startsWith('https://'),
      dbStatus: dbMsg,
      dbOk
    });
  }
  ...
  if (action === 'signup' && req.method === 'POST') {
    ...
    // ── 邀请奖励：邀请人 +100 慧根（被邀请人的 100 走注册奖励，实现"各得100"）──
    if (invite_code) {
      try {
        const code = String(invite_code).trim().toUpperCase();
        const inviters = await dbGet('user_profiles', { invite_code: code });
        const inviter = inviters?.[0];
        if (inviter && inviter.id && inviter.id !== adminData.id) {
          await dbUpdate('user_profiles', { id: inviter.id }, {
            credits: (inviter.credits ?? 0) + 100,
            total_earned: (inviter.total_earned ?? 0) + 100
          });
          await dbInsert('huigen_transactions', {
            user_id: inviter.id, amount: 100, type: 'invite_reward', description: '邀请好友奖励'
          }).catch(() => {});
        }
      } catch (e) {}
    }
  ...
  if (action === 'deduct' && req.method === 'POST') {
    ...
    if (!profile || !Array.isArray(profiles)) {
      ...
      // credits 为 NULL：signup bonus 未写入，补初始化（存量用户修复）
      await dbUpdate('user_profiles', { id: user_id }, { credits: 100, total_earned: 100 });
      await dbInsert('huigen_transactions', { user_id, amount: 100, type: 'signup_bonus', description: '注册奖励' }).catch(() => {});
      profile.credits = 100;
